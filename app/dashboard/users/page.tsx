...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 bg-gray-50 min-h-screen">
      ...
      {/* 등록 폼 (Create) */}
      <section className="bg-white p-5 md:p-6 rounded-xl border-2 border-blue-500 shadow-md">
        <h3 className="text-lg font-bold text-blue-700 mb-4">신규 등록</h3>

        {/* flex 대신 grid를 사용하면 모바일에서 정렬이 더 깔끔합니다 */}
        <form
          action={createUser}
          className="grid grid-cols-1 md:flex md:flex-row gap-3"
        >
          <input
            name="loginId"
            placeholder="아이디"
            className="border-2 border-gray-300 p-3 md:p-2 rounded-lg flex-1 text-gray-900 font-medium focus:border-blue-500 outline-none transition-all"
            required
          />
          <input
            name="name"
          ...
          {/* 모바일(w-full), PC(w-auto) 설정 */}
          <button
            type="submit"
            className="w-full md:w-auto bg-blue-700 text-white px-8 py-3 md:py-2 rounded-lg font-bold hover:bg-blue-800 active:scale-95 transition-all shadow-lg"
          >
            등록
          </button>
        </form>
